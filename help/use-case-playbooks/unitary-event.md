...
7. To ingest customer, user need to store `customer_country_code`, `customer_mobile_no`, `customer_firstname`, `customer_lastname` and `customer_email_id` in postman environment variables.
...
8. Update the Postman request **[!DNL `Customer Ingestion > Customer Streaming Ingestion`]** to change the preferred channel of customer by default [!DNL `email`] is configured in request.
```js
"consents": {
    "marketing": {
        "preferred": "email",
...
```
9. Trigger **[!DNL `Customer Ingestion > Customer Streaming Ingestion`]** to ingest the customer profile
